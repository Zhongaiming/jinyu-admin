...
import VTYPE from '@/api/atmosphere/videoType';
...
const loadDataTable = async (page: any, sort: any) => {
    return await VTYPE.get({
        videoTypeDtoFilter: { ...getSearchFieldsValue() },
        pageParam: { ...page },
        orderParam: [...sort],
    });
};
...
function confirmForm(form) {
    if (form) {
        if (postId.value != null) {
            VTYPE.update({ videoTypeDto: { id: postId.value, ...form } }).then(() => {
                window['$message'].success('编辑成功');
                modalRef.value.closeModal();
                reloadTable();
            });
        } else {
            VTYPE.add({ videoTypeDto: form }).then(() => {
