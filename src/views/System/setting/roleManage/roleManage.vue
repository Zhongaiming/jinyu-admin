<template>
  ...
  <n-card class="pro-card" hoverable>
    <BasicTable
      ...
      :row-key="(row:ListData) => row.roleId"
      ref="actionRef"
...
<script lang="ts" setup>
  ...
  import { getSysRoleList, deleteSysRole } from '@/api/System/setting/role';
  ...
  const searchSchemas: FormSchema[] = [
    {
      ...
      // labelMessage: '应用名称',
      component: 'NInput',
      label: I18n.global.t('jiaoSeMingCheng'),
      isShow: true,
      componentProps: {
        placeholder: I18n.global.t('qingShuRuJiaoSeMingCheng'),
        onInput: (e: any) => {
          console.log(e);
        },
      },
  ...
  const columns: BasicColumn<ListData>[] = [
    {
      title: I18n.global.t('jiaoSeId'),
      key: 'roleId',
      width: '200',
    },
    {
      title: I18n.global.t('jiaoSeMingCheng'),
      key: 'roleName',
    },
  ];
  ...
  // 新增角色
  function addTable() {
    router.push({ name: 'addEditRole' });
  }

  // 编辑角色
  function handleEdit(record: Recordable) {
    router.push({ name: 'addEditRole', query: { roleId: record.roleId } });
  }

  // 删除角色
  function handleDelete(record: Recordable) {
    window['$dialog'].info({
      title: I18n.global.t('shanChuJiaoSe'),
      content: I18n.global.t('queDingShanChuGaiJiaoSe'),
      positiveText: I18n.global.t('queDing'),
      negativeText: I18n.global.t('quXiao'),
      onPositiveClick: () => {
        deleteSysRole({ roleId: record.roleId }).then(() => {
