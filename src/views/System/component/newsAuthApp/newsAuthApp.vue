<template>
  <n-card
    ...
    :bordered="false"
    size="small"
    :title="$t('xinWenShouQuanYingYongLieBiao')"
  >
...
<script setup lang="ts">
  ...
  import {
    ...
    updateNewsApp,
    deleteNewsApp,
  } from '@/api/System/component/newsAuthApp';
  ...
  // 获取字典
  const { internal_app_dict, news_notify_type } = useDict('internal_app_dict', 'news_notify_type');
  ...
  function handleEdit(record: Recordable) {
    resetSubmitSchemas();
    getNewsAppView({ newsAppId: record.newsAppId }).then((res) => {
      newsAppId.value = record.newsAppId;
  ...
  function handleDelete(record: Recordable) {
    window['$dialog'].info({
      ...
      content: I18n.global.t('queDingQuXiaoGaiXinWenYingYongShouQuan'),
      positiveText: I18n.global.t('queDing'),
      negativeText: I18n.global.t('quXiao'),
      onPositiveClick: () => {
        deleteNewsApp({ newsAppId: record.newsAppId }).then(() => {
  ...
  function confirmForm(form) {
    if (form) {
      ...
      if (newsAppId.value != null) {
        updateNewsApp({
          newsAppDto: { newsAppId: newsAppId.value, ...form },
        }).then(() => {
        ...
        addNewsApp({ newsAppDto: form }).then(() => {
