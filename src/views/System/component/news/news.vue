    ...
    >
      <template #tableTitle>
        <n-button
          ...
          @click="router.push({ name: 'newsAuthApp' })"
        >
...
<script setup lang="ts">
  import { getAdminNewsList, readNews } from '@/api/Default/dashboard/news';
  ...
  // 获取字典
  const { news_type, news_notify_type, sys_user_dict, sys_dept_dict, internal_app_dict } = useDict(
    'news_type',
    'news_notify_type',
    'sys_user_dict',
    'sys_dept_dict',
  ...
  const columns: BasicColumn<ListData>[] = [
    ...
    },
    {
      ...
      key: 'type',
      render(row) {
        return getDictLabel(news_type.value, row.newsType);
      },
    },
    {
