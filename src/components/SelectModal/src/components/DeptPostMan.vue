<template>
  <n-grid y-gap="15" x-gap="15" cols="1 s:1 m:2 l:2 xl:2 2xl:2" responsive="screen">
    <n-gi>
      <n-card class="select-model-card">
    ...
    <n-gi>
      <n-card class="select-model-card">
        <div class="card-header flex items-center">
          ...
          <div class="flex-1">
            <n-input
              ...
              @keyup.enter="deptPostManLoadData"
            />
          </div>
        </div>
        <div
          v-for="item in deptPostManList"
          :key="item.userId"
          class="mt-2 man-item"
        ...
        <div class="flex items-center justify-center empty-box" v-if="!deptPostManList.length">
          <n-empty :description="$t('zanWuShuJu')" />
        </div>
      </n-card>
    </n-gi>
    <n-gi>
      <n-card class="select-model-card" v-if="selectList.length">
        <div class="flex items-center justify-between mb-2">
          <div class="font-semibold">{{ $t('yiXuanBuMenGangWeiRenYuan') }}</div>
...
<script lang="ts">
  ...
  import { getListByDeptPostId } from '@/api/System/setting/user';
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      // 获取部门岗位人员数据
      function deptPostManLoadData() {
        getListByDeptPostId({
          deptPostId: selectDeptPostId.value,
          name: userSearchKey.value,
        }).then((res) => {
          deptPostManList.value = res;
        });
      }
      ...
      function userSelect(item) {
        ...
        if (isMultiple) {
          selectList.value.push({
            label: `${selectDeptName.value} / ${selectPostName.value} / ${item.showName}`,
            value: item.userId,
          });
      ...
      // 全选
      function allChecked() {
        let allSelectList = deptPostManList.value.map((item) => {
          return {
            label: `${selectDeptName.value} / ${selectPostName.value} / ${item.showName}`,
            value: item.userId,
          };
