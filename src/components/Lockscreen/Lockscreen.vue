<template>
  ...
  >
    <template v-if="!showLogin">
      <div class="lock-box">
        <div class="lock">
          <span class="lock-icon" :title="$t('jieSuoPingMu')" @click="onLockLogin(true)">
            <n-icon>
              <lock-outlined />
            </n-icon>
          </span>
      ...
      <div class="local-time">
        ...
        <div class="date"
          >{{ month }}{{ $t('yue') }}{{ day }}{{ $t('hao') }}，{{ $t('xingQi') }}{{ week }}</div
        >
      </div>
    ...
    <template v-if="showLogin">
      <div class="login-box">
        ...
        <div class="username">{{ loginParams.loginName }}</div>
        <n-input
          type="password"
          autofocus
          v-model:value="loginParams.password"
          @keyup.enter="onLogin"
          :placeholder="$t('qingShuRuDengLuMiMa')"
        >
          <template #suffix>
        ...
        <div class="flex justify-around w-full mt-1">
          <div>
            <a @click="showLogin = false">{{ $t('fanHui') }}</a>
          </div>
          <div>
            <a @click="goLogin">{{ $t('zhongXinDengLu') }}</a>
          </div>
          <div>
            <a @click="onLogin">{{ $t('jinRuXiTong') }}</a>
          </div>
        </div>
      </div>
    </template>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const state = reactive({
        ...
        errorMsg: I18n.global.t('miMaCuoWu'),
        loginParams: {
          loginName: loginName || '',
          password: '',
        },
      ...
      const onLogin = async () => {
        ...
        params.password = encrypt(strTrim(params.password)) as string;
