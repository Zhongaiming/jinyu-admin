  ...
  function setColumns(columnList: string[]) {
    ...
    if (!isString(columns[0])) {
      ...
      columnList.forEach((key) => {
        if (key === 'selection') return newColumns.unshift({ type: 'selection', key: 'selection' });
        const columnItem = cacheColumns.find((item) => item.key === key);
        if (columnItem?.key) newColumns.push({ ...columnItem });
      });
