...
class Recorder {
  ...
  start(): Promise<{}> {
    ...
    return navigator.mediaDevices
      .getUserMedia({
        audio: this.config?.deviceId ? { deviceId: this.config.deviceId } : true,
      })
      .then(
        (stream) => {
          // audioInput表示音频源节点
  ...
  static getPermission(): Promise<{}> {
    this.initUserMedia();
    return navigator.mediaDevices
      .getUserMedia({ audio: this.config?.deviceId ? { deviceId: this.config?.deviceId } : true })
      .then((stream) => {
