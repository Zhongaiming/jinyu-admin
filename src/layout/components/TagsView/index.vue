...
<script lang="ts">
  ...
  import { CURRENT_APPID, TABS_ROUTES } from '@/store/mutation-types';
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const state = reactive({
        ...
        currentRouteIsAffix: route.meta.affix,
        currentRouteHasShortcut: route.meta.shortcut,
      });
      watch(
        () => route.path,
        () => {
          state.currentRouteIsAffix = route.meta.affix;
          state.currentRouteHasShortcut = route.meta.shortcut;
        },
        {
          deep: true,
        }
      ...
      const TabsMenuOptions = computed(() => {
        ...
        return [
          {
            label: I18n.global.t('shuaXinDangQian'),
            key: '1',
            icon: renderIcon(ReloadOutlined),
          },
          {
            label: I18n.global.t('guanBiDangQian'),
            key: '2',
            disabled: isCurrent.value || isDisabled || state.currentRouteIsAffix,
            icon: renderIcon(CloseOutlined),
          },
          {
            label: I18n.global.t('guanBiQiTa'),
            key: '3',
            disabled: isDisabled,
            icon: renderIcon(ColumnWidthOutlined),
          },
          {
            label: I18n.global.t('guanBiQuanBu'),
            key: '4',
            disabled: isDisabled,
            icon: renderIcon(MinusOutlined),
          },
          {
            label: I18n.global.t('tianJiaDaoKuaiJieFangShi'),
            key: '5',
            disabled: !state.currentRouteHasShortcut,
            icon: renderIcon(ApiTwotone),
          },
        ];
      });
      ...
      const removeTab = (route) => {
        if (tabsList.value.length === 1) {
          return message.warning(I18n.global.t('zheYiJingShiZuiHouYiYeBuNengZaiGuanBiLe'));
        }
      ...
      const closeAll = () => {
        ...
        const { tabsList } = tabsViewStore;
        router.replace({ name: tabsList.filter((item) => item.meta.affix)[0].name });
        updateNavScroll();
      };
      // 添加快捷方式
      const addShortcut = () => {
        const appId = storage.get(CURRENT_APPID);
        tabsViewStore.addShortcutTab(appId, route as any);
      };
      //tab 操作
